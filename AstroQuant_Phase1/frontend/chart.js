...
const AQ_DEFAULT_CHART_API_ORIGIN = String(window.location.port || "") === "8000"
...
const CHART_BOOTSTRAP_RETRY_MS = 1200;
...
function chartApiOrigins() {
	const preferred = String(window.AQ_API_BASE || AQ_DEFAULT_CHART_API_ORIGIN || "http://127.0.0.1:8000");
	return [preferred];
}
...
function startChartAutoLoadLoop() {
	if (window.__aqChartAutoLoadStarted) return;
	window.__aqChartAutoLoadStarted = true;

	const requestLoad = () => loadInstitutionalChart().catch(() => {});
	requestLoad();

	const bootstrapTimer = setInterval(() => {
		if (chartHasLoadedOnce) {
	...
	document.addEventListener("visibilitychange", () => {
		if (document.visibilityState === "visible" && !chartHasLoadedOnce) {
			requestLoad();
		}
	});
	...
	setInterval(requestLoad, CHART_AUTO_REFRESH_MS);
}

bindChartControls();
startChartAutoLoadLoop();
setInterval(() => {
