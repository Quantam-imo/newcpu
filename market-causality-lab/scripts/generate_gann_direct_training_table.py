...
- FIXED: moon_phase populated from canonical moon and eclipse events
...
def _extract_moon_phase_name(event_name: str) -> str | None:
    event_text = str(event_name or "")
    for label in MOON_PHASE_LABELS:
...
def _build_daily_event_features(gann_cycles: pd.DataFrame, gann_moon: pd.DataFrame, astro_nak: pd.DataFrame) -> pd.DataFrame:
    ...
    # Preserve canonical phase/ecliptic event names instead of collapsing them to Active Phase.
    gann_moon_full = gann_moon.copy()
    gann_moon_full["moon_phase_name"] = gann_moon_full["event"].apply(_extract_moon_phase_name)
